FROM envoyproxyplus/envoy-nginx-jaeger:{{envoy}}-{{nginx}}-{{jaeger}}-alpine
...
CMD sh -c "(fluentd -c /etc/fluent/fluent.conf &) && (envoy -c envoyproxy-config/envoy.yaml --log-path /tmp/envoyproxy.log &) && (nginx -g daemon off;)"